...
public class ChunkMesh {
		...
		for(int x=0;x<16;x++) {
			for(int y=0; y<256; y++) {
				for(int z=0; z<16; z++) {
					...
					if(!px) { for(int i=0;i<6;i++) { verticies.add(new Vertex(new Vector3f(PX_POS[i].x + x,PX_POS[i].y + y,PX_POS[i].z + z),cubes[chunk.cubes[x][y][z]].tc_px[i], NORMALS[i])); } }
					if(!nx) { for(int i=0;i<6;i++) { verticies.add(new Vertex(new Vector3f(NX_POS[i].x + x,NX_POS[i].y + y,NX_POS[i].z + z),cubes[chunk.cubes[x][y][z]].tc_nx[i], NORMALS[i])); } }
					if(!py) { for(int i=0;i<6;i++) { verticies.add(new Vertex(new Vector3f(PY_POS[i].x + x,PY_POS[i].y + y,PY_POS[i].z + z),cubes[chunk.cubes[x][y][z]].tc_py[i], NORMALS[i])); } }
					if(!ny) { for(int i=0;i<6;i++) { verticies.add(new Vertex(new Vector3f(NY_POS[i].x + x,NY_POS[i].y + y,NY_POS[i].z + z),cubes[chunk.cubes[x][y][z]].tc_ny[i], NORMALS[i])); } }
					if(!pz) { for(int i=0;i<6;i++) { verticies.add(new Vertex(new Vector3f(PZ_POS[i].x + x,PZ_POS[i].y + y,PZ_POS[i].z + z),cubes[chunk.cubes[x][y][z]].tc_pz[i], NORMALS[i])); } }
					if(!nz) { for(int i=0;i<6;i++) { verticies.add(new Vertex(new Vector3f(NZ_POS[i].x + x,NZ_POS[i].y + y,NZ_POS[i].z + z),cubes[chunk.cubes[x][y][z]].tc_nz[i], NORMALS[i])); } }
				}
			}
		}
	}
	...
	class Vertex{
		...
		Vector2f uvs;
		Vector3f normals;
		Vertex(Vector3f positions, Vector2f uvs, Vector3f normals){
