...
public class MousePicker {
	...
	public static void update() {
		...
		breakPos = search(Scene.currentScene.chunks,currentRay,Scene.currentScene.player.getGlobalPosition(), 0);
	...
	private static Vector3f search(List<Chunk> chunks, Vector3f ray, Vector3f position, int recursionCount) {
		Vector3f ppos = Scene.currentScene.player.getGlobalPosition();
		...
		return search(chunks,ray,new Vector3f(position.x+ray.x/4,position.y+ray.y/4,position.z+ray.z/4), recursionCount++);
	}
